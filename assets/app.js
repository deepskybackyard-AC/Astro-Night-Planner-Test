/* Astro Night Planner 1.4.0-test.1 – Testversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.4.0-test.1', release:'1.4.0-test.1', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.4.0-test.1'});
...
function renderAladinSurveySettings(c){
  ...
  const localRows=surveys.map(item=>{const entry=local.surveys[item.id]||{enabled:false,path:defaultLocalSurveyPath(item),lastStatus:'unchecked'};const full=joinSurveyUrl(local.baseUrl,entry.path);return `<details class="local-survey-row" data-local-survey-details="${esc(item.id)}" ${openLocalSurveyId===item.id?'open':''}><summary><strong>${esc(item.name)}</strong><span class="small muted">${esc(optionLabel(item.category||''))}</span></summary><div class="grid two" style="margin-top:10px"><label class="chip"><input data-local-survey="${esc(item.id)}" data-field="enabled" type="checkbox" ${entry.enabled?'checked':''}>${optionText('Lokale Quelle bevorzugen')}</label><label>${optionText('Lokaler relativer Pfad')}<input data-local-survey="${esc(item.id)}" data-field="path" value="${esc(entry.path)}"></label><label>${optionText('Online-Quelle')}<input value="${esc(item.hipsId)}" readonly></label><label>${optionText('Vollständige lokale URL')}<input value="${esc(full)}" readonly></label></div><div class="inline" style="margin-top:10px"><button type="button" data-test-local-survey="${esc(item.id)}">${optionText('Lokale Survey-Quelle prüfen')}</button><span class="small muted">${optionText('Status lokale Quelle')}: ${optionText(entry.lastStatus==='ok'?'Lokale Quelle erreichbar.':entry.lastStatus==='failed'?'Lokale Quelle nicht erreichbar.':'Nicht geprüft')}</span></div></details>`}).join('');
