/* Astro Night Planner 1.1.0-test.25 – Aladin-Tabs und Dämmerungslogik */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.1.0-test.25', release:'1.1.0-test.25', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.1.0-test.9'});
...
const DEFAULT_RELEASE_NOTES = {
  de: [
    ...
    'Aladin-Bedienung ist in Tabs Himmelsbild, Rahmen, Objekte sowie Zeit & Mond gegliedert.',
    'Dämmerungslogik: Astronomische Nacht wird nur angezeigt, wenn die Sonne am Standort tatsächlich unter -18° sinkt.',
    'Horizontansicht: Legende und Dämmerungsfarben passen wieder zur Darstellung.'
  ],
  en: [
    ...
    'Aladin controls are now grouped into the tabs sky image, frame, objects, and time & Moon.',
    'Twilight logic: astronomical night is shown only when the Sun actually drops below -18° at the location.',
    'Horizon view: legend and twilight colours now match the actual drawing.'
  ]
}
...
function drawHorizonChart(canvas,withTrack=false){
  ...
  if(withTrack&&track.length){
    ...
    context.strokeStyle='#38516d';context.lineWidth=1;context.strokeRect(margin.left,margin.top,plotW,plotH);context.fillStyle=palette.text;context.textAlign='left';context.textBaseline='top';context.fillText('Horizont (grün) · Objektbahn (gelb) · Dämmerungsphasen (blau) · Planungszeitraum (hellblau)',margin.left,2);
    return;
  }
  ...
  context.fillStyle=palette.text;context.textAlign='left';context.textBaseline='top';context.fillText('Persönlicher Horizont und Hindernisse',margin.left,2);
}
