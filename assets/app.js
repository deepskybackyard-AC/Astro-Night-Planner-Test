/* Astro Night Planner 1.4.1-test.1 – Testversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.4.1-test.1', release:'1.4.1-test.1', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.4.1-test.1'});
...
function renderInfo(){
  ...
  <div class="card help-article subtab-panel ${currentInfoSubTab==='help'?'active':''}"><div class="section-title-row"><div><h2>${esc(tx('helpTitle'))}</h2><div class="small muted">${esc(tx('helpSub'))}</div><div class="help-language-note">${esc(tx('helpNote'))}</div></div><div class="data-actions"><button type="button" data-open-help-section="help-storage">${esc(language==='en'?'Open integrated help':'Integrierte Hilfe öffnen')}</button><a class="button primary" href="${docLink('pdf')}" target="_blank" rel="noopener">${esc(tx('helpPdf'))}</a></div></div>
    ...
    <section id="help-local-surveys"><h3>Lokale Surveys und Local Survey Server</h3><p>Lokale Surveys sind optionale lokale Kopien von HiPS-Himmelskarten. Sie sind hilfreich, wenn ein Survey sehr groß ist, online langsam lädt oder auch ohne Internetverbindung verfügbar sein soll. Die App verwendet lokale Surveys über eine HTTP-Adresse wie <code>http://127.0.0.1:8765/</code>; normale Dateipfade können vom Browser und von Aladin nicht direkt als HiPS-Quelle genutzt werden.</p>
      <h4>Grundprinzip auf allen Betriebssystemen</h4><p>Der Local Survey Server stellt einen gemeinsamen Survey-Hauptordner als lokalen Webserver bereit. Der Planner liest dann <code>properties</code>, <code>Norder...</code>, <code>Dir...</code>, <code>Npix...</code> und <code>Allsky...</code> über URLs. Die Konfiguration in der App ist unter Windows, Linux und macOS gleich: Basis-URL <code>http://127.0.0.1:8765/</code> und pro Survey ein relativer Pfad wie <code>nsns/ohs8/</code>.</p>
      <h4>Windows</h4><p>Unter Windows wird das Paket <strong>ANP Local Survey Server 1.0</strong> empfohlen. Es bietet ein Windows-Kontrollfenster, Tray-Menü, Autostart, Hintergrundbetrieb, Start/Stop/Beenden und eine browserbasierte Konfigurationsoberfläche. Das Schließen per X versteckt das Kontrollfenster nur; <strong>Beenden</strong> stoppt das Hilfsprogramm vollständig und gibt die EXE frei.</p>
      <h4>Linux und macOS</h4><p>Für Linux und macOS gibt es ein separates natives Serverpaket mit Browser-Konfigurationsoberfläche. Wine wird nicht benötigt. Der Nutzer startet das passende Binary, zum Beispiel unter Linux <code>./bin/anp-local-survey-server-linux-amd64 --root "$HOME/AstroSurveys" --port 8765 --open</code> oder auf macOS <code>./bin/anp-local-survey-server-darwin-arm64 --root "$HOME/AstroSurveys" --port 8765 --open</code>. Die Oberfläche läuft normalerweise unter <code>http://127.0.0.1:8776/</code>. Für den regelmäßigen Einsatz kann unter Linux ein <code>systemd --user</code>-Dienst und unter macOS ein LaunchAgent verwendet werden.</p>
      <h4>Kann der Planner den Server starten?</h4><p>Eine Web-App kann aus Sicherheitsgründen keine lokalen Programme direkt starten. Der Button <strong>Local Survey Server öffnen</strong> kann nur die lokale Konfigurationsoberfläche öffnen, wenn der jeweilige Server bereits läuft. Wenn er nicht erreichbar ist, muss das Hilfsprogramm manuell gestartet oder per Autostart aktiviert werden.</p>
      <h4>Empfohlene Ordnerstruktur</h4><p>Verwende einen gemeinsamen Hauptordner, z. B. unter Windows <code>D:\AstroSurveys</code>, unter Linux <code>/home/benutzer/AstroSurveys</code> oder unter macOS <code>/Users/benutzer/AstroSurveys</code>. Darunter liegen die Survey-Ordner.</p><table><thead><tr><th>Ordner unterhalb des Hauptordners</th><th>Relativer Pfad im Planner</th><th>Bedeutung</th></tr></thead><tbody><tr><td><code>nsns/ohs8</code></td><td><code>nsns/ohs8/</code></td><td>NSNS OIII/H-alpha/SII-Komposit</td></tr><tr><td><code>nsns/halpha8</code></td><td><code>nsns/halpha8/</code></td><td>NSNS H-alpha</td></tr><tr><td><code>nsns/oiii8</code></td><td><code>nsns/oiii8/</code></td><td>NSNS OIII</td></tr><tr><td><code>nsns/sii8</code></td><td><code>nsns/sii8/</code></td><td>NSNS SII</td></tr><tr><td><code>nsns/hbr8</code></td><td><code>nsns/hbr8/</code></td><td>NSNS H-beta/Rot, sofern bereitgestellt</td></tr><tr><td><code>nsns/rgb8</code></td><td><code>nsns/rgb8/</code></td><td>NSNS RGB, sofern bereitgestellt</td></tr></tbody></table>
      <h4>Einrichtung im Planner</h4><ol><li>Starte den passenden Local Survey Server für dein Betriebssystem.</li><li>Prüfe im Browser, ob z. B. <code>http://127.0.0.1:8765/nsns/ohs8/properties</code> erreichbar ist.</li><li>Aktiviere im Planner <strong>Lokale Survey-Nutzung aktivieren</strong>.</li><li>Trage als Basis-URL <code>http://127.0.0.1:8765/</code> ein.</li><li>Aktiviere pro Survey die lokale Quelle und trage den relativen Pfad ein, z. B. <code>nsns/ohs8/</code>.</li><li>Nutze <strong>Lokale Survey-Quelle prüfen</strong>.</li></ol>
