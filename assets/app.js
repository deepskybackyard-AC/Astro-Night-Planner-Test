/* Astro Night Planner 1.0 – Testversion 18 mit NSNS-Surveys, Standortdialog, Aladin-Interaktionen und Umrisszeichnung */
'use strict';
  ...
  return `<details ${profile.central.collapsed.filters?'':'open'} id="filterDetails">
    ...
    <div class="filter-tab-panel ${active==='basis'?'active':''}" data-filter-tab-panel="basis">
      <div class="filters filter-main-grid">
        <label class="filter-search-field filter-area-search">Suche<input id="objectSearch" value="${esc(profile.planning.search)}" placeholder="M31, NGC, Rosette …"><span class="small muted">Automatische Aktualisierung nach 1,5 Sekunden; Enter sucht sofort.</span></label>
...
function renderLocations(){
  ...
  return`<div class="settings-tabs subtabs">${[['locations','Standorte'],['horizon','Horizontprofile']].map(([k,n])=>`<button data-locations-subtab="${k}" class="${currentLocationsSubTab===k?'active':''}">${n}</button>`).join('')}</div><div class="card subtab-panel ${currentLocationsSubTab==='locations'?'active':''}"><div class="section-title-row"><div><h2>Gespeicherte Aufnahmeorte</h2><div class="small muted">Der Standardstandort wird hier festgelegt. In der Planung kann vorübergehend ein anderer Ort gewählt werden.</div></div><button id="addLocation">Standort hinzufügen</button></div>
    ...
    ${showAddLocationDialog?`<div class="location-add-dialog"><div class="section-title-row"><div><h3>Neuen Standort hinzufügen</h3><div class="small muted">Suche einen Ort oder gib Koordinaten manuell ein. Nach dem Speichern wird der neue Standort aktiv ausgewählt.</div></div><button id="cancelAddLocation" type="button">Abbrechen</button></div><div class="grid three"><label>Name<input id="newLocName" value="${esc(pendingLocationDraft?.name||'Neuer Standort')}"></label><label>Zeitzone<select id="newLocTimezone">${timeZoneOptions(pendingLocationDraft?.timezone||loc.timezone||'Europe/Berlin')}</select></label><label>Höhe über Meer (m)<input id="newLocElevation" type="number" value="${Number(pendingLocationDraft?.elevation)||0}"></label><label>Breitengrad (°)<input id="newLocLat" type="number" step="0.0001" value="${Number(pendingLocationDraft?.latitude)||48.5}"></label><label>Längengrad (°)<input id="newLocLon" type="number" step="0.0001" value="${Number(pendingLocationDraft?.longitude)||9}"></label><div class="inline" style="align-self:end"><button id="saveNewLocation" type="button">Standort speichern</button></div></div></div>`:''}
...
function renderInfo(){
  ...
  <div class="card help-article subtab-panel ${currentInfoSubTab==='help'?'active':''}"><div class="section-title-row"><div><h2>${esc(tx('helpTitle'))}</h2><div class="small muted">${esc(tx('helpSub'))}</div><div class="help-language-note">${esc(tx('helpNote'))}</div></div><div class="data-actions"><a class="button" href="${docLink('html')}" target="_blank" rel="noopener">${esc(tx('helpBrowser'))}</a><a class="button primary" href="${docLink('pdf')}" target="_blank" rel="noopener">${esc(tx('helpPdf'))}</a></div></div>
    ...
    <section id="help-filters"><h3>Objektfilter</h3><p>Filtere nach Katalog, Objekttyp, Magnitude, Mindesthöhe, Sichtbarkeitsdauer, Mondabstand und Objektgröße. Texteingaben werden nach 1,5 Sekunden übernommen, damit nicht nach jedem Zeichen neu gerechnet wird; Enter oder „Filter anwenden“ startet sofort. Alle Beschriftungen enthalten die verwendeten Einheiten. Änderungen setzen die Ergebnisliste auf Seite 1 zurück.</p></section>
...
function wikipediaSearchUrl(object){
  ...
  const aliases=[object?.name, object?.aliases?.[0], object?.id].filter(Boolean);
  const query=aliases[0]||object?.id||'';
...
function bindRendered(){
  ...
  if(search){
    search.addEventListener('input',()=>{clearTimeout(objectFilterTimer);objectFilterTimer=setTimeout(applySearch,1500)});
...
function bindLocationDraft(){
  ...
  document.getElementById('addLocation')?.addEventListener('click',()=>{pendingLocationDraft=createEmptyLocationDraft();showAddLocationDialog=true;render()});
  ...
  document.getElementById('saveNewLocation')?.addEventListener('click',async()=>{const item=pendingLocationDraft||createEmptyLocationDraft();item.name=document.getElementById('newLocName')?.value?.trim()||'Neuer Standort';item.timezone=document.getElementById('newLocTimezone')?.value||'Europe/Berlin';item.latitude=Number(document.getElementById('newLocLat')?.value)||item.latitude;item.longitude=Number(document.getElementById('newLocLon')?.value)||item.longitude;item.elevation=Number(document.getElementById('newLocElevation')?.value)||0;draft.locations.push(item);draft.selectedLocationId=item.id;draft.central.defaultLocationId=item.id;showAddLocationDialog=false;pendingLocationDraft=null;horizonUndoStack=[];setSectionDirty('locations');await saveDraftSection('locations');profile.planning.locationId=item.id;await saveProfile();draft=deepClone(profile);render()});
