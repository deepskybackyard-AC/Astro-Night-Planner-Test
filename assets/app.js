/* Astro Night Planner 1.4.0-test.5 – Testversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.4.0-test.5', release:'1.4.0-test.5', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.4.0-test.5'});
...
function moonTransitDisplay(night,windowRange,loc){
  ...
  if(windowRange?.start&&date<windowRange.start)return `${base} – ${ui('liegt vor dem Planungszeitraum','before the planning window')}`;
  if(windowRange?.end&&date>windowRange.end)return `${base} – ${ui('liegt nach dem Planungszeitraum','after the planning window')}`;
  return base;
}
...
function openMultiNightWeatherWindow(){
  ...
  const sections=keys.map(key=>{const night=nightData(key,loc),range=planningWindow(night,profile.planning.planningWindow),q=nightQualityForDateKey(key,loc),moonLine=moonWindowLine(night,range,loc,{includeIllumination:true});return`<section class="multi-night-section"><h2>${fmtDate(key,loc.timezone)} · ${esc(range.label)}</h2><div class="small muted">${esc(loc.name)} · ${fmtTime(range.start,loc.timezone)}–${fmtTime(range.end,loc.timezone)} · Ø Qualität ${Number.isFinite(q)?fmt(q):'–'} · ${esc(moonLine)}</div>${hourlyWeatherRowsHtml(range,night,loc)}</section>`}).join('');
