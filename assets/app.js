/* Astro Night Planner 1.0 – Testversion 20 mit Umriss-Fixes, Popup-Schließen, Wikipedia-Spalte und UI-Korrekturen */
'use strict';
...
function preferredWikipediaTerm(object){
  const aliases=uniqueObjectTerms([object?.name,...(object?.aliases||[]),object?.id]);
  const common=aliases.find(value=>value&&!/^([A-Z]+\s*-?\s*\d+|M\s*\d+|NGC\s*\d+|IC\s*\d+)$/i.test(value));
  return common||aliases[0]||object?.id||'';
}
