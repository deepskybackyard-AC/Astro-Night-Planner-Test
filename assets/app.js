...
function auroraLevelFromKp(kp){const a=profile.central.aurora||{};if(!Number.isFinite(kp))return'none';if(kp>=Number(a.redKp??7.3))return'red';if(kp>=Number(a.orangeKp??6.7))return'orange';if(kp>=Number(a.yellowKp??5.7))return'yellow';return'none'}
function auroraLevelLabel(level){return{none:'keine Warnung',yellow:'erhöht',orange:'möglich',red:'stark möglich'}[level]||'keine Warnung'}
