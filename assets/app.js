/* Astro Night Planner 1.1.0-test.40 – originalgetreue eigene Karte im Wettervergleich */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.1.0-test.40', release:'1.1.0-test.40', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.1.0-test.40'});
...
const DEFAULT_RELEASE_NOTES = {
  de: [
    'Setup-Rahmen wird beim Objektwechsel automatisch wie „Rahmen auf Objekt zurücksetzen“ neu gesetzt.',
    ...
    'Aladin-Bedienung: Tabs bleiben in Arbeitsreihenfolge angeordnet; externer Himmelsbild-Button ist dauerhaft sichtbar.',
  ...
  en: [
    'When switching objects, the setup frame is automatically reset like using the reset-frame button.',
    ...
    'Aladin controls: tabs stay in workflow order; the external sky-image button remains permanently visible.',
...
Object.assign(EN_EXACT,{
  ...
  'Meteoblue-Zeit nicht zentral synchronisiert – Meteoblue-Zeitachse verwenden.':'Meteoblue time is not centrally synchronized - use the Meteoblue timeline.',
  'Gemeinsame Vergleichszeit steuert eigene Karte, Windy und Ventusky. Meteoblue verwendet die eigene Zeitachse.':'Shared comparison time controls the own map, Windy and Ventusky. Meteoblue uses its own timeline.',
  'Die eigene Astro-Wolkenkarte nutzt Modell, Wolkenschicht, Auswertung, Kartenansicht, Glättung und Overlays aus der Planung.':'The own astro cloud map uses model, cloud layer, evaluation, map view, smoothing and overlays from the planning view.',
...
Object.assign(EN_EXACT,{
  ...
  'Wettervergleich in neuem Tab öffnen':'Open weather comparison in new tab',
  'Gemeinsame Vergleichszeit':'Shared comparison time',
  'Vergleichszeit':'Comparison time',
  'Eigene Karte':'Own map',
  'Diese Zeit steuert die eigene Astro-Wolkenkarte. Externe Karten behalten ihre eigenen Zeitregler.':'This time controls the own astro cloud map. External maps keep their own time controls.',
  'Eigene Astro-Wolkenkarte mit Zeitsteuerung':'Own astro cloud map with time control',
  'Aktuelle Auswahl aus der Planung, steuerbar über den gemeinsamen Regler.':'Current selection from the planning view, controlled by the shared slider.',
  'Externe Karten verwenden ihre eigenen Zeitachsen; der gemeinsame Regler synchronisiert die eigene Karte.':'External maps use their own timelines; the shared slider synchronizes the own map.',
  'öffnest du am PC eine 2×2-Ansicht über die ganze Bildschirmbreite: links oben das eigene Astro-Wolkenmodell mit der aktuellen Auswahl, rechts oben Meteoblue, links unten Windy und rechts unten Ventusky. So erkennst du schnell, ob alle Quellen denselben Trend zeigen oder ob einzelne Modelle deutlich abweichen.':'opens a 2x2 full-width view on a PC: your own astro cloud model with the current selection in the top-left, Meteoblue in the top-right, Windy in the bottom-left and Ventusky in the bottom-right. This quickly shows whether all sources indicate the same trend or whether individual models differ clearly.',
...
function cloudMapSnapshotDataUrl(frameIndexOverride=null,width=1400,height=720,transparentMapBase=false,compact=false){
  const W=Math.max(480,Math.round(Number(width)||1400)),H=Math.max(260,Math.round(Number(height)||720));
  const canvas=document.createElement('canvas');canvas.width=W;canvas.height=H;
  const ctx=canvas.getContext('2d');
  const loc=activeLocation();
  if(!transparentMapBase){const gradient=ctx.createLinearGradient(0,0,0,H);gradient.addColorStop(0,'#071522');gradient.addColorStop(1,'#030b12');ctx.fillStyle=gradient;ctx.fillRect(0,0,W,H);}
  if(!cloudMapData){
    ctx.fillStyle='#dcecff';ctx.font='800 34px system-ui';ctx.textAlign='center';ctx.fillText(ui('Astro-Wolkenmodell','Astro cloud model'),W/2,H/2-20);
    ctx.font='500 22px system-ui';ctx.fillStyle='#9fbad2';ctx.fillText(ui('Noch keine Wolkenkarte geladen. Bitte in der Planung zuerst aktualisieren.','No cloud map has been loaded yet. Please refresh it in the planning view first.'),W/2,H/2+24);
    return canvas.toDataURL('image/png');
  }
  const settings=cloudMapSettings();
  const domValue=id=>document.getElementById(id)?.value||'';
  const view=domValue('cloudMapView')||currentCloudMapView();
  const layer=domValue('cloudMapLayer')||profile.planning.cloudMapLayer||settings.defaultLayer;
  const mode=domValue('cloudMapMode')||profile.planning.cloudMapMode||settings.defaultMode;
  const baseMode=domValue('cloudMapBaseMode')||currentCloudMapBaseMode();
  const compactMode=Boolean(compact);
  const frameIndex=clamp(Number(frameIndexOverride ?? profile.planning.cloudMapFrame)||0,0,cloudDisplayTimes().length-1),rawPosition=cloudRawPosition(frameIndex);
  const frame=cloudFrameValues(view,layer,rawPosition),drawValues=mode==='uncertainty'?frame.spread:frame.values;
  const size=cloudMapData.gridSize,map=compactMode?{x:0,y:0,w:W,h:H}:{x:88,y:54,w:W-176,h:H-126};
  ctx.save();ctx.beginPath();if(compactMode)ctx.rect(map.x,map.y,map.w,map.h);else ctx.roundRect(map.x,map.y,map.w,map.h,18);ctx.clip();
  const drawOwnBase=transparentMapBase&&mode!=='uncertainty'&&baseMode==='combined';
  if(!drawOwnBase){const base=ctx.createLinearGradient(map.x,map.y,map.x,map.y+map.h);base.addColorStop(0,'#152a22');base.addColorStop(1,'#07111d');ctx.fillStyle=base;ctx.fillRect(map.x,map.y,map.w,map.h);}
  const smoothing=domValue('cloudMapSmoothingTemporary')||currentCloudSmoothing(),offscreen=document.createElement('canvas'),renderWidth=smoothing==='structured'?760:smoothing==='soft'?90:390;
  ...
  for(let py=0;py<offscreen.height;py++){
    ...
    for(let px=0;px<offscreen.width;px++){
      ...
      offCtx.fillStyle=cloudMapColor(value,layer,mode);offCtx.fillRect(px,py,1,1);
    }
  }
  ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.save();ctx.filter=smoothing==='soft'?'blur(26px)':smoothing==='balanced'?'blur(2px)':'none';const pad=smoothing==='soft'?56:smoothing==='balanced'?4:0;ctx.drawImage(offscreen,map.x-pad,map.y-pad,map.w+pad*2,map.h+pad*2);ctx.restore();
  const weatherOverlays=cloudMapWeatherOverlays(),precip=cloudFrameValues(view,'precip',rawPosition).values,rain=cloudFrameValues(view,'rain',rawPosition).values,snow=cloudFrameValues(view,'snow',rawPosition).values;
  if(mode==='clouds'){
    if(weatherOverlays.precip){for(let py=0;py<map.h;py+=18){const gy=(py/map.h)*(size-1);for(let px=0;px<map.w;px+=18){const gx=(px/map.w)*(size-1),value=cloudMapInterpolated(precip,size,gx,gy);if(value>.03){ctx.strokeStyle=`rgba(83,190,255,${clamp(.35+value/2,.35,.95)})`;ctx.lineWidth=2.4;ctx.beginPath();ctx.moveTo(map.x+px+4,map.y+py-4);ctx.lineTo(map.x+px-5,map.y+py+10);ctx.stroke()}}}}
    if(weatherOverlays.rain){for(let py=4;py<map.h;py+=24){const gy=(py/map.h)*(size-1);for(let px=6;px<map.w;px+=24){const gx=(px/map.w)*(size-1),value=cloudMapInterpolated(rain,size,gx,gy);if(value>.02){ctx.strokeStyle=`rgba(32,139,255,${clamp(.38+value/2,.38,.95)})`;ctx.lineWidth=2.8;ctx.beginPath();ctx.moveTo(map.x+px,map.y+py);ctx.lineTo(map.x+px-3,map.y+py+9);ctx.stroke()}}}}
    if(weatherOverlays.snow){ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='700 15px system-ui';for(let py=10;py<map.h;py+=30){const gy=(py/map.h)*(size-1);for(let px=10;px<map.w;px+=30){const gx=(px/map.w)*(size-1),value=cloudMapInterpolated(snow,size,gx,gy);if(value>.01){ctx.fillStyle=`rgba(188,112,255,${clamp(.42+value/2,.42,.98)})`;ctx.fillText('✦',map.x+px,map.y+py)}}}}
  }
  ctx.strokeStyle='rgba(135,190,225,.20)';ctx.lineWidth=1.2;for(let i=1;i<4;i++){ctx.beginPath();ctx.ellipse(map.x+map.w/2,map.y+map.h/2,map.w*i/8,map.h*i/8,0,0,Math.PI*2);ctx.stroke()}
  ctx.restore();if(!compactMode){ctx.strokeStyle='rgba(95,190,240,.72)';ctx.lineWidth=2;ctx.strokeRect(map.x,map.y,map.w,map.h);}
  if(cloudMapValuesVisible()){ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=`800 ${size>=9?18:20}px system-ui`;const labelStep=size>=9?2:1;for(let row=0;row<size;row++)for(let col=0;col<size;col++){if(row!==Math.floor(size/2)||col!==Math.floor(size/2))if(row%labelStep||col%labelStep)continue;const index=row*size+col;const value=Number(frame.values[index]);if(!Number.isFinite(value))continue;const x=map.x+col/Math.max(1,size-1)*map.w,y=map.y+row/Math.max(1,size-1)*map.h,text=`${Math.round(value)}%`;ctx.lineWidth=6;ctx.strokeStyle='rgba(2,7,12,.92)';ctx.strokeText(text,x,y);ctx.fillStyle='#f2ad55';ctx.fillText(text,x,y)}}
  const cx=map.x+map.w/2,cy=map.y+map.h/2;ctx.fillStyle='#ffd66b';ctx.strokeStyle='#06111b';ctx.lineWidth=5;ctx.beginPath();ctx.arc(cx,cy,10,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.strokeStyle='#ffd66b';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(cx-22,cy);ctx.lineTo(cx+22,cy);ctx.moveTo(cx,cy-22);ctx.lineTo(cx,cy+22);ctx.stroke();
  const movement=estimateCloudMovement(view,layer,frameIndex);if(movement?.reliable&&Number.isFinite(movement.azimuth)&&movement.distance>=1){const length=clamp(60+movement.distance*2,70,170),angle=toRad(movement.azimuth),dx=Math.sin(angle)*length,dy=-Math.cos(angle)*length;ctx.strokeStyle='#ffcf5b';ctx.fillStyle='#ffcf5b';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+dx,cy+dy);ctx.stroke();const head=14,headAngle=Math.atan2(dy,dx);ctx.beginPath();ctx.moveTo(cx+dx,cy+dy);ctx.lineTo(cx+dx-head*Math.cos(headAngle-.55),cy+dy-head*Math.sin(headAngle-.55));ctx.lineTo(cx+dx-head*Math.cos(headAngle+.55),cy+dy-head*Math.sin(headAngle+.55));ctx.closePath();ctx.fill()}
  if(!compactMode){
...
function openWeatherComparisonWindow(){
  const loc=activeLocation();
  const mb=meteoblueLocationInfo(loc),urls=externalWeatherUrls(loc);
  const displayTimes=cloudMapData?cloudDisplayTimes():[];
  const frameCount=Math.max(1,displayTimes.length||1);
  const frameIndex=clamp(Number(profile.planning.cloudMapFrame)||0,0,frameCount-1);
  const step=cloudMapTimeStepMinutes();
  const settings=cloudMapSettings();
  const mapMode=document.getElementById('cloudMapMode')?.value||profile.planning.cloudMapMode||settings.defaultMode||'clouds';
  const baseMode=document.getElementById('cloudMapBaseMode')?.value||currentCloudMapBaseMode();
  const ownUsesBase=Boolean(cloudMapData&&mapMode!=='uncertainty'&&baseMode==='combined');
  const snapshot=cloudMapSnapshotDataUrl(frameIndex,1400,720,ownUsesBase,false);
  ...
  const externalFrames=frameTimes.map(value=>{const t=new Date(value);const u=externalWeatherUrls(loc,t);return{meteoblue:mb.mapWidget,windy:u.windyEmbed,ventusky:u.ventuskyEmbed,label:fmtDateTime(t,loc.timezone)}});
  const place=loc?.name||ui('Planungsstandort','Planning location');
  const title=ui('Wettervergleich','Weather comparison');
  const cloudTitle=ui('Eigene Astro-Wolkenkarte','Own astro cloud map');
  const subtitle=ui('Nutzt Modell, Wolkenschicht, Auswertung, Kartenansicht, Glättung und Overlays aus der Planung. Die Zeit wird oben zentral gesteuert.','Uses model, cloud layer, evaluation, map view, smoothing and overlays from the planning view. Time is controlled centrally above.');
  const mbTitle=ui('Meteoblue Windanimation','Meteoblue wind animation');
  const windyTitle='Windy';
  const ventuskyTitle='Ventusky';
  const popup=window.open('','_blank');
  if(!popup){alert(ui('Der Browser hat das neue Fenster blockiert. Bitte Pop-ups für diese Seite erlauben.','The browser blocked the new window. Please allow pop-ups for this site.'));return}
  const safe=(value)=>String(value||'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const json=(value)=>JSON.stringify(value).replace(/</g,'\u003c');
  const initialLabel=timeLabels[frameIndex]||'–';
  const locInfo={lat:Number(loc.latitude)||0,lon:Number(loc.longitude)||0,radius:Number(cloudMapData?.radiusKm)||120};
  const maplibreHead=ownUsesBase?'<link rel="stylesheet" href="https://unpkg.com/maplibre-gl@5.24.0/dist/maplibre-gl.css"><script src="https://unpkg.com/maplibre-gl@5.24.0/dist/maplibre-gl.js"><\/script>':'';
  popup.document.open();
  popup.document.write(`<!DOCTYPE html><html lang="${language==='en'?'en':'de'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${safe(title)} - ${safe(place)}</title>${maplibreHead}<style>
    :root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#07111d;color:#ecf6ff;font-family:system-ui,-apple-system,Segoe UI,sans-serif}header{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:12px 18px;background:#0d1d2d;border-bottom:1px solid #27445f}h1{font-size:22px;margin:0 0 4px}p{margin:0;color:#aecaeb}.comparison-timebar{min-width:420px;max-width:780px;flex:1;display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;background:#081726;border:1px solid #27445f;border-radius:14px;padding:10px 12px}.comparison-timebar label{display:grid;gap:4px;color:#aecaeb;font-weight:700}.comparison-timebar strong{color:#fff;font-size:18px}.comparison-timebar input,.own-map-controls input{width:100%;accent-color:#8ec8ff}.comparison-timebar button,.own-map-controls button{border:1px solid #355c7e;background:#173250;color:#fff;border-radius:10px;padding:9px 12px;font-weight:800}.grid{width:100vw;height:calc(100vh - 106px);display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:8px;padding:8px}.panel{min-width:0;min-height:0;border:1px solid #27445f;border-radius:14px;overflow:hidden;background:#0b1826;display:flex;flex-direction:column}.panel h2{font-size:17px;line-height:1.2;margin:0;padding:10px 12px;background:#10263c;border-bottom:1px solid #27445f}.panel .note{font-size:13px;color:#9fbad2;font-weight:500}.panel .sync-warning{color:#ff7575;font-weight:800}.panel iframe{width:100%;height:100%;border:0;display:block;object-fit:initial;background:#06111c}.panel img{border:0;display:block}.own-map-controls{display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;padding:8px 10px;border-bottom:1px solid #27445f;background:#081726}.own-map-time{font-weight:800;color:#ffe08a;text-align:center}.own-map-stage{position:relative;min-height:0;flex:1;background:#07100f;overflow:hidden;display:flex;align-items:center;justify-content:center;padding:8px}.own-map-frame{position:relative;width:100%;height:100%;max-width:100%;max-height:100%;background:#07100f;overflow:hidden;box-shadow:inset 0 0 38px rgba(0,0,0,.28)}.own-map-base{position:absolute;left:6.2857%;right:6.2857%;top:7.5%;bottom:10%;z-index:0;border-radius:16px;overflow:hidden;background:#17231f;box-shadow:inset 0 0 0 1px rgba(135,175,196,.17),inset 0 0 45px rgba(0,0,0,.16)}.own-map-base[hidden]{display:none}.own-map-base .maplibregl-map{width:100%;height:100%;background:#17231f}.own-map-base .maplibregl-canvas{filter:brightness(.82) saturate(.86) contrast(1.18)}.own-map-base::after{content:'';position:absolute;inset:0;z-index:2;pointer-events:none;background:linear-gradient(180deg,rgba(3,9,10,.03),rgba(3,9,10,.09))}.own-map-base .maplibregl-control-container{position:relative;z-index:3}.own-map-base .maplibregl-ctrl-attrib{font-size:9px;color:#d1dbe3;background:rgba(4,10,13,.74);border-radius:5px 0 0 0}.own-map-base .maplibregl-ctrl-attrib a{color:#c9e2f5}.own-map-overlay{position:absolute;inset:0;z-index:1;width:100%;height:100%;object-fit:contain;background:transparent;pointer-events:none}.source-link{color:#9fd0ff;text-decoration:none}.source-link:hover{text-decoration:underline}@media(max-width:1100px){header{align-items:stretch;flex-direction:column}.comparison-timebar{min-width:0;width:100%}.grid{height:auto;grid-template-columns:1fr;grid-template-rows:none}.panel{height:70vh}header{position:sticky;top:0;z-index:2}}
  </style></head><body><header><div><h1>${safe(title)} - ${safe(place)}</h1><p>${safe(ui('Vier Kontrollkarten für den aktuellen Planungsstandort. Für die parallele Ansicht ist ein PC oder großer Bildschirm sinnvoll.','Four reference maps for the current planning location. A PC or large screen is recommended for the parallel view.'))}<br>${safe(ui('Gemeinsame Vergleichszeit steuert eigene Karte, Windy und Ventusky. Meteoblue verwendet die eigene Zeitachse.','Shared comparison time controls the own map, Windy and Ventusky. Meteoblue uses its own timeline.'))}</p></div><div class="comparison-timebar"><button id="cmpPrev" type="button">−${step} min</button><label>${safe(ui('Gemeinsame Vergleichszeit','Shared comparison time'))}<strong id="cmpTimeLabel">${safe(initialLabel)}</strong><input id="cmpSlider" type="range" min="0" max="${Math.max(0,frameCount-1)}" step="1" value="${frameIndex}" ${cloudMapData?'':'disabled'}></label><button id="cmpNext" type="button">+${step} min</button></div></header><main class="grid"><section class="panel"><h2>${safe(cloudTitle)} <span class="note">· ${safe(subtitle)}</span></h2><div class="own-map-stage"><div id="ownMapFrame" class="own-map-frame"><div id="ownMapBase" class="own-map-base" ${ownUsesBase?'':'hidden'}></div><img id="ownCloudMap" class="own-map-overlay" src="${snapshot}" alt="${safe(cloudTitle)}"></div></div></section><section class="panel"><h2>${safe(mbTitle)} <span class="note sync-warning" id="statusMeteoblue">${safe(ui('Meteoblue-Zeit nicht zentral synchronisiert – Meteoblue-Zeitachse verwenden.','Meteoblue time is not centrally synchronized - use the Meteoblue timeline.'))}</span></h2><iframe id="meteoblueCompareFrame" title="${safe(mbTitle)}" src="${safe(mb.mapWidget)}" loading="eager" allow="geolocation" sandbox="allow-same-origin allow-scripts allow-popups allow-popups-to-escape-sandbox allow-forms" referrerpolicy="strict-origin-when-cross-origin"></iframe></section><section class="panel"><h2>${safe(windyTitle)} <span class="note" id="statusWindy">${safe(ui('bestmögliche Übergabe','best effort handoff'))}</span></h2><iframe id="windyCompareFrame" title="Windy" src="${safe(externalFrames[frameIndex]?.windy||urls.windyEmbed)}" loading="eager" sandbox="allow-same-origin allow-scripts allow-popups allow-popups-to-escape-sandbox allow-forms" referrerpolicy="strict-origin-when-cross-origin"></iframe></section><section class="panel"><h2>${safe(ventuskyTitle)} <span class="note" id="statusVentusky">${safe(ui('bestmögliche Übergabe','best effort handoff'))}</span></h2><iframe id="ventuskyCompareFrame" title="Ventusky" src="${safe(externalFrames[frameIndex]?.ventusky||urls.ventuskyEmbed)}" loading="eager" sandbox="allow-same-origin allow-scripts allow-popups allow-popups-to-escape-sandbox allow-forms" referrerpolicy="strict-origin-when-cross-origin"></iframe></section></main><script>
    ...
    const ownUsesBase=${json(ownUsesBase)};
    const locInfo=${json(locInfo)};
    ...
    function snapshotFor(index){try{if(window.opener&&typeof window.opener.cloudMapSnapshotDataUrl==='function')return window.opener.cloudMapSnapshotDataUrl(index,1400,720,ownUsesBase,false)}catch(error){} return fallback}
    ...
    function setFrameUrls(frame,force=false){const item=externalFrames[frame]||externalFrames[0]||{}; const label=labels[frame]||item.label||''; [['windyCompareFrame','windy','statusWindy'],['ventuskyCompareFrame','ventusky','statusVentusky']].forEach(([frameId,key,statusId])=>{const iframe=$(frameId); if(!iframe||!item[key])return; setText(statusId,statusLoading); const next=cacheBust(item[key],frame+'_'+key); if(force||iframe.src!==next)iframe.src=next; setTimeout(()=>setText(statusId, statusDone+' · '+(label||statusLimited)),900);});}
    function scheduleExternalSync(){clearTimeout(externalTimer); externalTimer=setTimeout(()=>setFrameUrls(current,false),700)}
    function setFrame(index,external=false){current=Math.max(0,Math.min(max,Math.round(Number(index)||0))); const label=labels[current]||'–'; ['cmpSlider'].forEach(id=>{const el=$(id); if(el)el.value=String(current)}); setText('cmpTimeLabel',label); const img=$('ownCloudMap'); if(img)img.src=snapshotFor(current); if(external)scheduleExternalSync()}
    function resizeOwnMapFrame(){const stage=document.querySelector('.own-map-stage'),frame=$('ownMapFrame'); if(!stage||!frame)return; const ratio=35/18,availableW=Math.max(1,stage.clientWidth-16),availableH=Math.max(1,stage.clientHeight-16); let w=availableW,h=availableW/ratio; if(h>availableH){h=availableH;w=h*ratio} frame.style.width=Math.round(w)+'px'; frame.style.height=Math.round(h)+'px'; if(ownBaseMap)requestAnimationFrame(()=>ownBaseMap&&ownBaseMap.resize());}
    function initOwnBaseMap(){resizeOwnMapFrame(); const maplibreApi=window.maplibregl||(window.opener&&window.opener.maplibregl); if(!ownUsesBase||!maplibreApi)return; const element=$('ownMapBase'); if(!element)return; const radius=Number(locInfo.radius)||120,lat=Number(locInfo.lat)||0,lon=Number(locInfo.lon)||0,latDelta=radius/111,lonDelta=radius/(111*Math.max(.2,Math.cos(lat*Math.PI/180))); const applyStyle=()=>{try{const hide=/(building|house|poi|transit|rail|aeroway|airport|parking|ferry|shop|amenity|industrial)/i,minor=/(path|track|service|minor|residential|street)/i; for(const layer of ownBaseMap.getStyle().layers||[]){const id=String(layer.id||''),source=String(layer['source-layer']||''),key=id+' '+source; if(hide.test(key)){ownBaseMap.setLayoutProperty(id,'visibility','none');continue} if(layer.type==='symbol'){if(!/(place|country|state|city|town|village|settlement)/i.test(key)){ownBaseMap.setLayoutProperty(id,'visibility','none');continue} ownBaseMap.setPaintProperty(id,'text-opacity',.78); ownBaseMap.setPaintProperty(id,'text-color','#b8d1e3'); ownBaseMap.setPaintProperty(id,'text-halo-color','#071018'); ownBaseMap.setPaintProperty(id,'text-halo-width',1.2)}else if(layer.type==='line'){if(minor.test(key))ownBaseMap.setPaintProperty(id,'line-opacity',.16); else ownBaseMap.setPaintProperty(id,'line-opacity',.38)}else if(layer.type==='background'){ownBaseMap.setPaintProperty(id,'background-color','#18231f')}else if(layer.type==='fill'){ownBaseMap.setPaintProperty(id,'fill-opacity',.56)}}}catch(error){}}; try{ownBaseMap=new maplibreApi.Map({container:element,style:'https://tiles.openfreemap.org/styles/liberty',center:[lon,lat],attributionControl:true,interactive:false,preserveDrawingBuffer:false,fadeDuration:0}); ownBaseMap.fitBounds([[lon-lonDelta,lat-latDelta],[lon+lonDelta,lat+latDelta]],{padding:0,duration:0}); ownBaseMap.on('style.load',()=>{applyStyle();setTimeout(()=>ownBaseMap&&ownBaseMap.resize(),120)}); ownBaseMap.on('load',()=>{applyStyle();setTimeout(()=>ownBaseMap&&ownBaseMap.resize(),120)}); window.addEventListener('resize',resizeOwnMapFrame); setTimeout(resizeOwnMapFrame,50);}catch(error){element.hidden=true;}}
    ['cmpSlider'].forEach(id=>{const el=$(id); if(el){el.addEventListener('input',event=>setFrame(event.target.value,true));el.addEventListener('change',()=>setFrameUrls(current,true));}});
    $('cmpPrev')?.addEventListener('click',()=>{setFrame(current-1,false);setFrameUrls(current,true)}); $('cmpNext')?.addEventListener('click',()=>{setFrame(current+1,false);setFrameUrls(current,true)});
    initOwnBaseMap();
  <\/script></body></html>`);
...
function bindRendered(){
  ...
  document.getElementById('openWeatherComparison')?.addEventListener('click',openWeatherComparisonWindow);
