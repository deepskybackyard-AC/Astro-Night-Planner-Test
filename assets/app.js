/* Astro Night Planner 1.1.0-test.39 – finale Korrektur Wettervergleich */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.1.0-test.39', release:'1.1.0-test.39', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.1.0-test.39'});
...
function openWeatherComparisonWindow(){
  ...
  const snapshot=cloudMapSnapshotDataUrl(frameIndex,1200,720,ownUsesBase,true);
  ...
  popup.document.write(`<!DOCTYPE html><html lang="${language==='en'?'en':'de'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${safe(title)} - ${safe(place)}</title>${maplibreHead}<style>
    ...
    function snapshotFor(index){try{if(window.opener&&typeof window.opener.cloudMapSnapshotDataUrl==='function')return window.opener.cloudMapSnapshotDataUrl(index,1200,720,ownUsesBase,true)}catch(error){} return fallback}
