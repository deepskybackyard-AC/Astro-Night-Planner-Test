/* Astro Night Planner 1.0 – Testversion 15 mit Setup-Kombinationen, Filter-Tabs und erweiterten Aladin-Funktionen */
'use strict';
...
const CURATED_OBJECT_OUTLINES = {
  'NGC7000': {source:'Manual planning outline, curated from publicly viewable sky imagery; approximate, not a scientific boundary.', polygons:[[
    [314.10,43.12],[314.28,44.05],[314.68,44.78],[315.05,45.38],[315.45,45.06],[316.08,44.58],[316.35,44.05],[316.00,43.50],[315.58,43.02],[315.34,42.55],[314.82,42.25],[314.35,42.35],[314.08,42.75]
  ]]},
  'M42': {source:'Manual planning outline, approximate.', polygons:[[[83.55,-5.65],[83.70,-5.22],[83.95,-5.15],[84.08,-5.45],[83.95,-5.85],[83.70,-5.98]]]}
};
...
function renderFraming(o,windowRange,loc){
  ...
  const outline=curatedOutlineForObject(o),survey=profile.planning.aladinSurvey||'P/DSS2/color';
...
function bindFraming(){
  ...
  document.getElementById('openAladinExternal')?.addEventListener('click',()=>{const frame=document.getElementById('aladinFrame');if(frame?.src)window.open(frame.src+'&external=1','_blank','noopener')});
...
function bindTargetSettings(){
  ...
  if(targetSearch){
    targetSearch.addEventListener('input',()=>{clearTimeout(targetFilterTimer);targetFilterTimer=setTimeout(applyTargetFilter,900)});
