/* Astro Night Planner 1.4.0-test.7 – Testversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.4.0-test.7', release:'1.4.0-test.7', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.4.0-test.7'});
...
function renderFraming(o,windowRange,loc){
  ...
  const temporaryCombinationNotice=setup&&!setup.isSavedCombination?`<div class="notice subtle temporary-equipment-notice">${ui('Diese Kombination ist nicht als Setup gespeichert. Die Rahmung wird temporär aus Teleskop, Reducer/Korrektor und Kamera berechnet.','This combination is not saved as a setup. The framing is calculated temporarily from telescope, reducer/corrector and camera.')} <button type="button" class="secondary" disabled>${ui('Als Setup speichern folgt','Save as setup to follow')}</button></div>`:'';
...
function renderInfo(){
  ...
  <div class="card help-article subtab-panel ${currentInfoSubTab==='help'?'active':''}"><div class="section-title-row"><div><h2>${esc(tx('helpTitle'))}</h2><div class="small muted">${esc(tx('helpSub'))}</div><div class="help-language-note">${esc(tx('helpNote'))}</div></div><div class="data-actions"><button type="button" data-open-help-section="help-storage">${esc(language==='en'?'Open integrated help':'Integrierte Hilfe öffnen')}</button><a class="button primary" href="${docLink('pdf')}" target="_blank" rel="noopener">${esc(tx('helpPdf'))}</a></div></div>
    ...
    <section id="help-local-surveys"><h3>${optionText('Lokale Surveys und Local Survey Server')}</h3>${language==='en'?`
      <p>Online surveys remain configured with their normal HiPS ID or online URL. Local surveys are an optional addition: the app first tries to use the local copy and only uses the online source if the fallback option is enabled and the local source is not available.</p>
      <h4>Why a local server is needed</h4><p>A browser cannot read a Windows folder path such as <code>D:\\AstroSurveys\\nsns\\ohs8</code> directly. The files must be made available through a local HTTP address. The recommended address is <code>http://127.0.0.1:8765/</code>. This address points to your own computer; it is not an internet server.</p>
      <h4>Recommended folder structure</h4><p>Create one common root folder, for example <code>D:\\AstroSurveys</code>. Below this root folder, create or let the Windows helper create survey folders such as <code>nsns\\ohs8</code>, <code>nsns\\halpha8</code>, <code>nsns\\oiii8</code>, <code>nsns\\sii8</code>, <code>nsns\\hbr8</code> or <code>nsns\\rgb8</code>. The folder names correspond to the short codes in the online NSNS HiPS address. For example <code>simg.de/P/NSNS/DR0_2/ohs8</code> becomes the local relative path <code>nsns/ohs8/</code>.</p>
      <h4>Using the Windows helper</h4><ol><li>Download and unzip the <strong>Astro Night Planner Local Survey Server 1.0</strong>.</li><li>Start the configuration program and select the root folder, for example <code>D:\\AstroSurveys</code>.</li><li>Select the surveys you want to keep locally. The helper downloads the complete HiPS folder structure, including the <code>properties</code> file and the <code>Norder...</code> tile folders.</li><li>Start the server program. Leave it running while you use local surveys in the planner.</li><li>In the planner enter <code>http://127.0.0.1:8765/</code> as the local survey base URL.</li><li>For each survey enable the local source and check the relative path, for example <code>nsns/ohs8/</code>.</li></ol>
      <h4>How the app decides between local and online</h4><p>If local survey usage and “prefer local sources” are enabled, the app checks the local <code>properties</code> file and then loads the HiPS tiles from the local server. If the local server is not reachable and online fallback is enabled, the app uses the configured online HiPS source. If online fallback is disabled, the app shows an error instead of silently switching to a different background survey.</p>
      <h4>What the check button does</h4><p>The “Check local survey source” button verifies that the configured URL can be reached and that a HiPS <code>properties</code> file is available. For a complete display, the survey must also contain the matching <code>Allsky</code> and <code>Norder...</code> tile files. The app reads the tile format from <code>hips_tile_format</code>, for example <code>png</code>, and must not force a different format.</p>
      <h4>Day buttons and Moon information</h4><p>The day buttons show a compact Moon summary for the selected planning window. The value after <code>▲</code> is the maximum Moon altitude during the planning window, not necessarily the Moon culmination of the whole night. In the planning-night cards the Moon culmination is shown for the sunset-to-sunrise night; if it lies before or after the selected planning window, the card explicitly says so.</p>`:`
      <p>Online-Surveys bleiben mit ihrer normalen HiPS-ID oder Online-URL konfiguriert. Lokale Surveys sind eine optionale Ergänzung: Die App versucht zuerst, die lokale Kopie zu verwenden, und nutzt die Online-Quelle nur dann, wenn der Fallback aktiviert ist und die lokale Quelle nicht erreichbar ist.</p>
      <h4>Warum ein lokaler Server erforderlich ist</h4><p>Ein Browser kann einen Windows-Ordnerpfad wie <code>D:\\AstroSurveys\\nsns\\ohs8</code> nicht direkt lesen. Die Dateien müssen über eine lokale HTTP-Adresse bereitgestellt werden. Empfohlen ist <code>http://127.0.0.1:8765/</code>. Diese Adresse zeigt auf den eigenen Computer; sie ist kein Internetserver.</p>
      <h4>Empfohlene Ordnerstruktur</h4><p>Lege einen gemeinsamen Hauptordner an, zum Beispiel <code>D:\\AstroSurveys</code>. Darunter liegen die Survey-Ordner, die entweder manuell angelegt oder vom Windows-Hilfsprogramm erzeugt werden, zum Beispiel <code>nsns\\ohs8</code>, <code>nsns\\halpha8</code>, <code>nsns\\oiii8</code>, <code>nsns\\sii8</code>, <code>nsns\\hbr8</code> oder <code>nsns\\rgb8</code>. Die Ordnerkürzel entsprechen den Kürzeln in der Online-HiPS-Adresse. Aus <code>simg.de/P/NSNS/DR0_2/ohs8</code> wird lokal der relative Pfad <code>nsns/ohs8/</code>.</p>
      <h4>Einrichtung mit dem Windows-Hilfsprogramm</h4><ol><li>Lade den <strong>Astro Night Planner Local Survey Server 1.0</strong> herunter und entpacke ihn.</li><li>Starte das Konfigurationsprogramm und wähle den Hauptordner, zum Beispiel <code>D:\\AstroSurveys</code>.</li><li>Wähle die Surveys aus, die lokal gespeichert werden sollen. Das Hilfsprogramm lädt die vollständige HiPS-Struktur mit <code>properties</code> und den <code>Norder...</code>-Kachelordnern.</li><li>Starte das Serverprogramm. Es muss laufen, solange lokale Surveys im Planner genutzt werden.</li><li>Trage im Planner als lokale Survey-Basis-URL <code>http://127.0.0.1:8765/</code> ein.</li><li>Aktiviere pro Survey die lokale Quelle und prüfe den relativen Pfad, zum Beispiel <code>nsns/ohs8/</code>.</li></ol>
      <h4>Entscheidung zwischen lokal und online</h4><p>Wenn die lokale Survey-Nutzung und „Lokale Quellen bevorzugen“ aktiviert sind, prüft die App zuerst die lokale <code>properties</code>-Datei und lädt anschließend die HiPS-Kacheln vom lokalen Server. Wenn der lokale Server nicht erreichbar ist und der Online-Fallback aktiviert ist, wird die konfigurierte Online-HiPS-Quelle verwendet. Wenn der Online-Fallback deaktiviert ist, zeigt die App eine Fehlermeldung, statt still auf einen anderen Hintergrundsurvey umzuschalten.</p>
      <h4>Was der Prüfbutton macht</h4><p>Der Button „Lokale Survey-Quelle prüfen“ prüft, ob die konfigurierte URL erreichbar ist und ob eine HiPS-<code>properties</code>-Datei vorhanden ist. Für die vollständige Anzeige müssen außerdem die passenden <code>Allsky</code>- und <code>Norder...</code>-Kacheldateien vorhanden sein. Die App liest das Kachelformat aus <code>hips_tile_format</code>, zum Beispiel <code>png</code>, und darf kein anderes Format erzwingen.</p>
      <h4>Tagesbuttons und Mondinformationen</h4><p>Die Tagesbuttons zeigen eine kompakte Mondübersicht für den ausgewählten Planungszeitraum. Die Angabe hinter <code>▲</code> ist die maximale Mondhöhe während des Planungszeitraums, nicht zwingend die Mondkulmination der gesamten Nacht. In den Planungsdaten wird die Mondkulmination für die Nacht von Sonnenuntergang bis Sonnenaufgang angezeigt; wenn sie vor oder nach dem gewählten Planungszeitraum liegt, wird dies ausdrücklich ergänzt.</p>`}</section><section id="help-horizon"><h3>Standorte und interaktive Horizontprofile</h3><p>Für jeden Standort können mehrere Horizontprofile angelegt werden. Die Standortsuche zeigt eine Trefferliste mit Ort, Region, Land, Postleitzahl und Koordinaten; ein bevorzugtes Land verhindert, dass eine deutsche PLZ ungeprüft einem weltweiten Ersttreffer zugeordnet wird. Die Zeitzone wird automatisch ermittelt und kann nur aus gültigen IANA-Zeitzonen gewählt werden.</p><p>Ein Horizontprofil wird als Standard festgelegt; in Planung und Detailansicht kann vorübergehend ein anderes gewählt werden. Bearbeite die Linie direkt mit Maus oder Finger. Hindernisse gehören zum jeweiligen Profil.</p></section>
...
function bindFraming(){
  ...
  const changeEquipment=async(type,value)=>{if(type==='scope')profile.planning.temporaryTelescopeId=value===profile.equipment.selectedTelescopeId?null:value;else if(type==='camera')profile.planning.temporaryCameraId=value===profile.equipment.selectedCameraId?null:value;else if(type==='optical')profile.planning.temporaryOpticalAccessoryId=value===(activeSetup()?.opticalAccessoryId||profile.equipment.selectedOpticalAccessoryId||OPTICAL_ACCESSORY_NONE_ID)?null:value;const object=catalog.find(item=>item.id===profile.planning.selectedObjectId);if(object&&framingSettings().autoRotate)profile.planning.frameRotation=optimalFrameRotation(object);await saveProfile();render()};
