/* Astro Night Planner 1.1.0-test.37 – finalisierter Wettervergleich */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.1.0-test.37', release:'1.1.0-test.37', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.1.0-test.37'});
...
function cloudMapSnapshotDataUrl(frameIndexOverride=null,width=1400,height=720,transparentMapBase=false){
  ...
  const settings=cloudMapSettings(),view=currentCloudMapView(),layer=profile.planning.cloudMapLayer||settings.defaultLayer,mode=profile.planning.cloudMapMode||settings.defaultMode,baseMode=currentCloudMapBaseMode();
  ...
  const size=cloudMapData.gridSize,map={x:88,y:54,w:W-176,h:H-126};
  ctx.save();ctx.beginPath();ctx.roundRect(map.x,map.y,map.w,map.h,18);ctx.clip();
  ...
  const smoothing=currentCloudSmoothing(),offscreen=document.createElement('canvas'),renderWidth=smoothing==='structured'?760:smoothing==='soft'?90:390;
  ...
  ctx.restore();ctx.strokeStyle='rgba(95,190,240,.72)';ctx.lineWidth=2;ctx.strokeRect(map.x,map.y,map.w,map.h);
...
function openWeatherComparisonWindow(){
  ...
  const settings=cloudMapSettings();
  const mapMode=profile.planning.cloudMapMode||settings.defaultMode||'clouds';
  const baseMode=currentCloudMapBaseMode();
  const ownUsesBase=Boolean(cloudMapData&&mapMode!=='uncertainty'&&baseMode==='combined');
  const snapshot=cloudMapSnapshotDataUrl(frameIndex,1000,520,ownUsesBase);
  ...
  const subtitle=ui('Die eigene Astro-Wolkenkarte nutzt Modell, Wolkenschicht, Auswertung, Kartenansicht, Glättung und Overlays aus der Planung.','The own astro cloud map uses model, cloud layer, evaluation, map view, smoothing and overlays from the planning view.');
  ...
  popup.document.write(`<!DOCTYPE html><html lang="${language==='en'?'en':'de'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${safe(title)} - ${safe(place)}</title>${maplibreHead}<style>
    ...
    function snapshotFor(index){try{if(window.opener&&typeof window.opener.cloudMapSnapshotDataUrl==='function')return window.opener.cloudMapSnapshotDataUrl(index,1000,520,ownUsesBase)}catch(error){} return fallback}
