/* Astro Night Planner 1.1.0-test.38 – finalisierter Wettervergleich */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.1.0-test.38', release:'1.1.0-test.38', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.1.0-test.38'});
...
function openWeatherComparisonWindow(){
  ...
  popup.document.write(`<!DOCTYPE html><html lang="${language==='en'?'en':'de'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${safe(title)} - ${safe(place)}</title>${maplibreHead}<style>
    ...
    function initOwnBaseMap(){if(!ownUsesBase||!window.maplibregl)return; const element=$('ownMapBase'); if(!element)return; const radius=Number(locInfo.radius)||120,lat=Number(locInfo.lat)||0,lon=Number(locInfo.lon)||0,latDelta=radius/111,lonDelta=radius/(111*Math.max(.2,Math.cos(lat*Math.PI/180))); try{ownBaseMap=new maplibregl.Map({container:element,style:'https://tiles.openfreemap.org/styles/liberty',center:[lon,lat],attributionControl:true,interactive:false,preserveDrawingBuffer:false,fadeDuration:0}); ownBaseMap.fitBounds([[lon-lonDelta,lat-latDelta],[lon+lonDelta,lat+latDelta]],{padding:0,duration:0}); ownBaseMap.on('load',()=>{setTimeout(()=>ownBaseMap&&ownBaseMap.resize(),120)}); window.addEventListener('resize',()=>ownBaseMap&&ownBaseMap.resize());}catch(error){element.hidden=true;}}
