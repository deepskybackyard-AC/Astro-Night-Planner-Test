/* Astro Night Planner 1.3.0-test.4 – Testversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.3.0-test.4', release:'1.3.0-test.4', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.3.0-test.4'});
...
function standardProfile(){
  return {
    ...
    central:{
      ...
      externalAladin:{altAzGrid:false,altAzGridColor:'#1f6f52',altAzGridLineWidth:0.8,groundMode:'standard',groundOpacity:50,compass:true,orientation:'standard'},
...
function normalizeProfile(p){
  ...
  out.central.externalAladin.groundOpacity=clamp(Number(out.central.externalAladin.groundOpacity??50),0,95);
...
function renderCentral(){
  ...
  <div class="card subtab-panel ${currentCentralSubTab==='display'?'active':''}">
    ...
    <details class="aladin-horizon-settings" style="margin-top:12px"><summary><span class="disclosure-arrow" aria-hidden="true"></span><strong>Aladin - Himmel & Horizont</strong><span class="small muted">Boden/Horizont, azimutales App-Raster und Himmelsrichtungen für den externen Aladin-Tab.</span></summary>
      <div class="grid three" style="margin-top:12px"><label>Bodenanzeige im externen Aladin-Tab<select id="externalAladinGroundMode"><option value="none" ${String(c.externalAladin?.groundMode||'standard')==='none'?'selected':''}>kein Boden</option><option value="standard" ${String(c.externalAladin?.groundMode||'standard')==='standard'?'selected':''}>Standardhorizont (0°)</option><option value="personal" ${String(c.externalAladin?.groundMode||'standard')==='personal'?'selected':''}>persönlicher Horizont</option></select></label><label>Transparenz Boden (%)<input id="externalAladinGroundOpacity" type="number" min="0" max="95" step="5" value="${Number(c.externalAladin?.groundOpacity??50)}"></label><label class="chip"><input id="externalAladinAltAzGrid" type="checkbox" ${c.externalAladin?.altAzGrid===true?'checked':''}>Azimutales Gradnetz als App-Overlay anzeigen</label><label class="color-picker-field">Farbe azimutales Gradnetz<span class="color-picker-row"><span id="externalAladinAltAzGridColorSwatch" class="color-swatch" style="background:${esc(c.externalAladin?.altAzGridColor||'#1f6f52')}"></span><input id="externalAladinAltAzGridColor" type="color" value="${esc(c.externalAladin?.altAzGridColor||'#1f6f52')}"></span></label><label>Linienstärke azimutales Gradnetz (px)<input id="externalAladinAltAzGridLineWidth" type="number" min="0.5" max="3" step="0.1" value="${Number(c.externalAladin?.altAzGridLineWidth??0.8)}"></label><label class="chip"><input id="externalAladinCompass" type="checkbox" ${c.externalAladin?.compass!==false?'checked':''}>Himmelsrichtungen/Kompassmarken anzeigen</label></div>
    </details></div>
...
function bindCentralDraft(){
  ...
  const groundOpacity=document.getElementById('externalAladinGroundOpacity');if(groundOpacity)groundOpacity.onchange=()=>{draft.central.externalAladin=draft.central.externalAladin||{};draft.central.externalAladin.groundOpacity=clamp(Number(groundOpacity.value)||50,0,95);setSectionDirty('display')};
  const altAzColor=document.getElementById('externalAladinAltAzGridColor');if(altAzColor){const swatch=document.getElementById('externalAladinAltAzGridColorSwatch');const update=()=>{draft.central.externalAladin=draft.central.externalAladin||{};draft.central.externalAladin.altAzGridColor=altAzColor.value;if(swatch)swatch.style.background=altAzColor.value;setSectionDirty('display')};altAzColor.oninput=()=>{if(swatch)swatch.style.background=altAzColor.value};altAzColor.onchange=update;}
...
async function saveDraftSection(section){
  ...
  if(section==='display'){
    ...
    draft.central.externalAladin=draft.central.externalAladin||standardProfile().central.externalAladin;draft.central.externalAladin.groundMode=['none','standard','personal'].includes(draft.central.externalAladin.groundMode)?draft.central.externalAladin.groundMode:'standard';draft.central.externalAladin.groundOpacity=clamp(Number(draft.central.externalAladin.groundOpacity??50),0,95);draft.central.externalAladin.altAzGridLineWidth=clamp(Number(draft.central.externalAladin.altAzGridLineWidth??0.8),0.5,3);if(!/^#[0-9a-fA-F]{6}$/.test(String(draft.central.externalAladin.altAzGridColor||'')))draft.central.externalAladin.altAzGridColor='#1f6f52';profile.central.defaultPlanningWindow=draft.central.defaultPlanningWindow;profile.planning.planningWindow=draft.central.defaultPlanningWindow;profile.central.framing=deepClone(draft.central.framing);profile.central.qualityThresholds=deepClone(draft.central.qualityThresholds);profile.central.aladinLabels=deepClone(draft.central.aladinLabels);profile.central.aladinSurveys=deepClone(draft.central.aladinSurveys);profile.central.externalAladin=deepClone(draft.central.externalAladin||standardProfile().central.externalAladin);profile.central.miniHorizon=deepClone(draft.central.miniHorizon||standardProfile().central.miniHorizon);profile.central.listDisplay=deepClone(draft.central.listDisplay);profile.central.frameVisible=draft.central.frameVisible;profile.central.objectSizeVisible=draft.central.objectSizeVisible;profile.central.meteoblueCollapsed=draft.central.meteoblueCollapsed;profile.central.detailPanels=deepClone(draft.central.detailPanels);profile.central.collapsed=deepClone(draft.central.collapsed);profile.central.visibleSections=deepClone(draft.central.visibleSections||{});profile.planning.detailsOpen=false;page=1;dirtySections.delete('display');
  }
  if(section==='locations'){
