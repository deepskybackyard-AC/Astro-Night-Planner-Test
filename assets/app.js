/* Astro Night Planner 1.0 – Testversion 12 mit Korrekturen Zielkatalog, Filterlayout und Objektliste */
'use strict';
...
function computeObjects(window,night,weather){const p=profile.planning,needle=p.search.trim().toLocaleLowerCase('de'),loc=activeLocation();const durationWindow=visibilityBasisWindow(night);const minSize=Number(p.minSize)||0,maxSize=Number(p.maxSize)||0;return catalog
  ...
  .filter(o=>Array.isArray(p.types)&&p.types.length&&p.types.includes(o.type))
...
function renderTargets(){
  ...
  const rows=targets.map(target=>{const o=catalog.find(item=>item.id===target.objectId);if(!o)return`<div class="target-row" data-target-row="${esc(target.id)}"><strong>${esc(target.objectId)}</strong><span class="warn">Objekt nicht im Katalog gefunden.</span><button data-delete-target="${esc(target.id)}" class="danger">Entfernen</button></div>`;const f=framingAnalysis(o);return`<div class="target-row" data-target-row="${esc(target.id)}"><div class="target-main"><strong>${esc(o.id)} · ${esc(o.name)}</strong><span>${esc(o.type)} · ${esc(o.constellation)} · Größe ${objectSizeText(o)} · Mag. ${o.magnitude??'–'} · Flächenhelligkeit ${o.surfaceBrightness??'–'}</span><span>Filter: ${(o.recommendedFilters||[]).join(', ')||'keine Angabe'} · Beste Monate: ${bestMonthsText(o)} · Rahmung aktuell: ${framingStatusLabel(f.code||f.status)} · Maximalhöhe wird in der Planung standortbezogen berechnet.</span></div><label>Priorität<select data-target-field="priority"><option ${target.priority==='niedrig'?'selected':''}>niedrig</option><option ${target.priority==='normal'||!target.priority?'selected':''}>normal</option><option ${target.priority==='hoch'?'selected':''}>hoch</option></select></label><label>Status<select data-target-field="status">${['Wunschziel','geplant','begonnen','Daten vollständig','bearbeitet','abgeschlossen','erneut aufnehmen'].map(v=>`<option ${target.status===v?'selected':''}>${v}</option>`).join('')}</select></label><label>Notiz<textarea data-target-field="note" rows="2" placeholder="Eigene Hinweise, Ziel, Palette, Setup, offene Punkte">${esc(target.note||'')}</textarea></label><label>Referenzbild-Links<textarea data-target-field="referenceLinksText" rows="2" placeholder="Ein Link pro Zeile">${esc((target.referenceLinks||[]).join('\n'))}</textarea></label><button data-delete-target="${esc(target.id)}" class="danger">Entfernen</button></div>`}).join('')||'<div class="muted">Noch keine Aufnahmeziele gespeichert. Füge Ziele in der Objektliste oder über die Suche hinzu.</div>';
  return`<div class="card"><div class="section-title-row"><div><h2>Meine Aufnahmeziele</h2><div class="small muted">Persönlicher Zielkatalog für die Frage: Was kann ich heute aufnehmen, was ich schon immer aufnehmen wollte?</div></div><button id="openTargetSearch" type="button">Aufnahmeziel suchen</button></div><div class="notice">Dieser Katalog erscheint in der Planung als eigener Katalogfilter und ist initial nicht aktiv. Wenn nur „Meine Aufnahmeziele“ gewählt ist, gelten weiterhin alle normalen Filter wie Wetter, Mond, Rahmung, Größe und Mindestbewertung.</div><div id="targetAddMessage" class="small muted" style="margin-top:10px"></div><div class="target-list">${rows}</div>${renderTargetSearchDialog()}</div>`;
}
