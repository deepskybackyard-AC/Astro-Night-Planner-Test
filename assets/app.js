/* Astro Night Planner 1.4.0-test.4 – Testversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.4.0-test.4', release:'1.4.0-test.4', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.4.0-test.4'});
...
Object.assign(EN_EXACT,{
  ...
  'Der optionale Local Survey Server kann lokale HiPS-Dateien ohne Python bereitstellen und bekannte NSNS-HiPS-Surveys vollständig herunterladen.':'The optional Local Survey Server can serve local HiPS files without Python and download known NSNS HiPS surveys completely.',
...
function resolvedAladinSurvey(item,source=profile){const cfg=normalizeLocalSurveySettings(source?.central?.localSurveys, normalizeAladinSurveys(source?.central?.aladinSurveys)),entry=item?cfg.surveys[item.id]:null;return cfg.enabled&&cfg.preferLocal&&entry?.enabled&&entry.path?localSurveyUrlFor(item,source):item?.hipsId||''}
...
function renderFraming(o,windowRange,loc){
  ...
  const surveyDebug=`${optionText('Tatsächlich geladene Survey-Quelle')}: ${surveyIsLocal?optionText('lokal'):optionText('online')} · ${survey}`;
...
function renderInfo(){
  ...
  <div class="card help-article subtab-panel ${currentInfoSubTab==='help'?'active':''}"><div class="section-title-row"><div><h2>${esc(tx('helpTitle'))}</h2><div class="small muted">${esc(tx('helpSub'))}</div><div class="help-language-note">${esc(tx('helpNote'))}</div></div><div class="data-actions"><button type="button" data-open-help-section="help-storage">${esc(language==='en'?'Open integrated help':'Integrierte Hilfe öffnen')}</button><a class="button primary" href="${docLink('pdf')}" target="_blank" rel="noopener">${esc(tx('helpPdf'))}</a></div></div>
    ...
    <section id="help-local-surveys"><h3>${optionText('Lokale Surveys und Local Survey Server')}</h3><p>${language==='en'?'Online surveys are still configured with their HiPS ID or online URL. For offline planning the app can optionally use a local HiPS source that is served by a local web server. A normal file path such as D:\\AstroSurveys\\nsns\\ohs8 cannot be read directly by the browser. Use a local HTTP address such as http://127.0.0.1:8765/.':'Online-Surveys bleiben mit ihrer HiPS-ID oder Online-URL konfiguriert. Für Offline-Planung kann die App optional eine lokale HiPS-Quelle nutzen, die von einem lokalen Webserver bereitgestellt wird. Ein normaler Dateipfad wie D:\\AstroSurveys\\nsns\\ohs8 kann vom Browser nicht direkt gelesen werden. Verwende eine lokale HTTP-Adresse wie http://127.0.0.1:8765/.'}</p><p>${language==='en'?'The test package includes the optional Windows helper “Astro Night Planner Local Survey Server”. It does not require Python. It can be configured once, started by double-click and can download known NSNS HiPS surveys completely, validate them and serve them locally.':'Das Testpaket enthält das optionale Windows-Hilfsprogramm „Astro Night Planner Local Survey Server“. Es benötigt keine Python-Installation. Es wird einmal konfiguriert, per Doppelklick gestartet und kann bekannte NSNS-HiPS-Surveys vollständig herunterladen, validieren und lokal bereitstellen.'}</p><p>${language==='en'?'A single local server is sufficient for all surveys if they are stored below one common root folder. Example: D:\\AstroSurveys contains nsns/ohs8, nsns/halpha8 and dss2/red. Start the local server for the root folder and enter http://127.0.0.1:8765/ as the local survey base URL. Per survey only the relative path is needed, for example nsns/ohs8/.':'Ein einziger lokaler Server reicht für alle Surveys, wenn sie unter einem gemeinsamen Hauptordner liegen. Beispiel: D:\\AstroSurveys enthält nsns/ohs8, nsns/halpha8 und dss2/red. Starte den lokalen Server für diesen Hauptordner und trage http://127.0.0.1:8765/ als lokale Survey-Basis-URL ein. Pro Survey wird nur der relative Pfad benötigt, z. B. nsns/ohs8/.'}</p><p>${language==='en'?'The “Check local survey source” button only checks whether the configured URL is reachable and whether a HiPS properties file can be requested. The helper contains a complete download mode for known NSNS HiPS surveys. Interrupted downloads can be started again; existing files are skipped.':'Der Button „Lokale Survey-Quelle prüfen“ prüft nur, ob die konfigurierte URL erreichbar ist und ob eine HiPS-properties-Datei angefragt werden kann. Das Hilfsprogramm enthält einen vollständigen Downloadmodus für bekannte NSNS-HiPS-Surveys. Abgebrochene Downloads können erneut gestartet werden; vorhandene Dateien werden übersprungen.'}</p></section><section id="help-horizon"><h3>Standorte und interaktive Horizontprofile</h3><p>Für jeden Standort können mehrere Horizontprofile angelegt werden. Die Standortsuche zeigt eine Trefferliste mit Ort, Region, Land, Postleitzahl und Koordinaten; ein bevorzugtes Land verhindert, dass eine deutsche PLZ ungeprüft einem weltweiten Ersttreffer zugeordnet wird. Die Zeitzone wird automatisch ermittelt und kann nur aus gültigen IANA-Zeitzonen gewählt werden.</p><p>Ein Horizontprofil wird als Standard festgelegt; in Planung und Detailansicht kann vorübergehend ein anderes gewählt werden. Bearbeite die Linie direkt mit Maus oder Finger. Hindernisse gehören zum jeweiligen Profil.</p></section>
...
function bindCentralDraft(){
  ...
  async function testLocalUrl(url,id){if(id)openLocalSurveyId=id;if(!url){alert(language==='en'?'No local URL configured.':'Keine lokale URL konfiguriert.');return}const test=url.replace(/\/+$/,'')+'/properties';try{const response=await fetch(test,{method:'GET',cache:'no-store',headers:{'Cache-Control':'no-cache'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);if(id){const cfg=ensureLocal();cfg.surveys[id].lastStatus='ok';cfg.surveys[id].lastChecked=new Date().toISOString();setSectionDirty('display');render()}else alert(language==='en'?'Local source reachable.':'Lokale Quelle erreichbar.')}catch(error){if(id){const cfg=ensureLocal();cfg.surveys[id].lastStatus='failed';cfg.surveys[id].lastChecked=new Date().toISOString();setSectionDirty('display');render()}else alert(language==='en'?'Local source unavailable. The app cannot start the local web server.':'Lokale Quelle nicht erreichbar. Die App kann den lokalen Webserver nicht starten.')}}
