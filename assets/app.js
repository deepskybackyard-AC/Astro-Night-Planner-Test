/* Astro Night Planner 1.4.2-test.3 – Testversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'test', appVersion:'1.4.2-test.3', release:'1.4.2-test.3', databaseName:'astro-night-planner-test-v1', documentTitle:'Astro Night Planner 1.4.2-test.3'});
...
Object.assign(EN_EXACT,{
  ...
  'Öffnet die Downloadverwaltung des laufenden Local Survey Servers. Dort können bekannte NSNS-Surveys sowie weitere kompatible HiPS-Surveys über eine eigene Quelladresse und einen relativen Zielpfad geladen, pausiert, fortgesetzt, abgebrochen und geprüft werden.':'Opens the download manager of the running Local Survey Server. Known NSNS surveys and other compatible HiPS surveys can be downloaded from a custom source URL into a relative target path, paused, resumed, cancelled and validated there.',
  ...
  'Der optionale Local Survey Server 1.1.2 kann lokale HiPS-Dateien ohne Python bereitstellen und bekannte NSNS- sowie weitere kompatible HiPS-Surveys herunterladen.':'The optional Local Survey Server 1.1.2 can serve local HiPS files without Python and download known NSNS and other compatible HiPS surveys.',
  'Local Survey Server öffnen':'Open Local Survey Server',
  'Local Survey Server nicht erreichbar. Starte ANP-Local-Survey-Server-1.1.2.exe oder aktiviere den Autostart.':'Local Survey Server not reachable. Start ANP-Local-Survey-Server-1.1.2.exe or enable autostart.',
...
function bindCentralDraft(){
  ...
  async function selectLocalSurveyFolder(id){
    ...
    try{
      ...
      const numbered=folders.map((folder,index)=>`${index+1}. ${folder}${folder===current?' *':''}`).join('\n');
      const answer=prompt(`${optionText('Survey-Ordner auswählen')}:

${numbered}

1-${folders.length}`,String(Math.max(1,folders.indexOf(current)+1)));
      if(answer===null)return;const index=Number(answer)-1;if(!Number.isInteger(index)||!folders[index])return;
      const cfg=ensureLocal(),entry=cfg.surveys[id]||(cfg.surveys[id]={enabled:false,path:'',lastStatus:'unchecked'});entry.path=folders[index];entry.enabled=true;entry.lastStatus='unchecked';setSectionDirty('display');render();
